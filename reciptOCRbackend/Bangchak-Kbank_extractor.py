...
def extract_data(image_pil, original_filename, initial_result):
    ...
    global_regex_patterns = {
        "date": r"(?:DATE|Date|วันที่พิมพ์|มือจ่าย|วันที่ขาย)[:\s]*(\d{1,2}/\d{1,2}/(?:\d{4}|\d{2}))",
        ...
        "gas_tax_id": r'(?:เสียภาษี|TAX ID)[:\s]*(\d{10,15})',
        "merchant_name": r"(?:บริษัท.*?จำกัด)|(?:ห้างหุ้นส่วนจำกัด)|(?:bangchak.*?co\.?,?\s*ltd\.)|(?:bangchak)",
        "total_amount": r"(?:TOTAL|SALE)?\s*THB[:\s]*(?P<money_amount>\d{1,3}(?:,\d{3})*\.\d{2})",
        "gas_type": r"(DIESEL|E20|E85|GASOHOL|HI DIESEL)",
        "gas_address": r"(?:HEAD OFFICE|ที่อยู่|address)[:\s]*(?P<captured_text>.*?\b\d{5}\b)",
        "plate_no": r'(?:ทะเบียนรถ|เบียนรถ)[:\s]*(?P<plate_no>(?:[ก-ฮ]{1,2}|[1-9])(?:[ก-ฮ]{1,2}|\d{1,4}){1,2})',
        "milestone": r'(?:เลขระยะทาง)[:\s]*.*?(\d{8})',
        "receipt_no": r"(?:mid)[\s:#(]*(\d{15})",
        "liters": r"(?:LITER|liter)[:\s]*(\d+\.\d{2})",
    }
