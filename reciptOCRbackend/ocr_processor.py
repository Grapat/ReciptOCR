...
def dynamic_parse_ocr(image_pil, receipt_type="dynamic"):
    ...
    keywords = {
        'merchant_name': ['PTT', 'BANGCHAK', 'PTTST.D', 'บริษัท'],
        'date': ['date', 'DATE', 'วันที่ขาย'],
        'total_amount': ['AMT THB:', 'TOTAL THB:', 'VOID SALE', 'Amount'],
        'gas_provider': ['PTTstation', 'bangchak', 'PTT'],
        'gas_address': ['address', 'NAAME & ADDRESS'],
        'gas_tax_id': ['tax id', 'เลขประจำตัวผู้เสียภาษี', 'เลขประจำตัว'],
        'receipt_no': ['receipt no', 'TID'],
        'liters': ['liters', 'liter', 'Ltrs'],
        'plate_no': ['ทะเบียนรถ', 'plate no', 'รถ'],
        'milestone': ['milestone', 'เลขไมล์', 'กม.'],
        'VAT': ['vat', 'ภาษีมูลค่าเพิ่ม'],
        'gas_type': ['e20', 'e85', 'gasohol', 'DIESEL', 'HI DIESEL S', 'แก๊สโซฮอล์', '91', '95'],
        # Keywords for EGAT TH address
        'egat_address_th': ['การไฟฟ้าฝ่ายผลิตแห่งประเทศไทย', 'กฟผ', 'กฟผ.', 'นนทบุรี', 'บางกรวย'],
        # Keywords for EGAT ENG address
        'egat_address_eng': ['Electricity Generating Authority of Thailand', 'EGAT', 'Nonthaburi', 'Bang Kruai'],
        # Keywords for EGAT Tax ID
        'egat_tax_id': ['tax id', 'เลขประจำตัวผู้เสียภาษีกฟผ'],
    }
                ...
                if field in ["total_amount", "VAT", "liters", "milestone"]:
                    ...
                    if field == "egat_tax_id" or field == "gas_tax_id":
                        ...
                        if tax_id_match:
                            ...
                            # If no direct number found, try to use the next word if it looks like an ID
                            if next_word_idx < len(data['text']) and re.search(r'^\d[\d\s\-]{8,18}\d$', data['text'][next_word_idx].strip()):
                                value = data['text'][next_word_idx].strip().replace(
                                    " ", "").replace("-", "")
                            else:
                                value = None  # Fallback to None if nothing matches
                    elif field == "plate_no":
                    ...
                    if field == 'gas_provider' and value != "N/A":
                        if 'ptt' in value.lower():
                            ...
                            value = "N/A"  # Default if not recognized ENUM
    ...
    # --- Clean raw extracted text here ---
    # Replace multiple whitespaces (spaces, tabs, newlines) with a single space
    cleaned_extracted_text = re.sub(r'\s', '', extracted_text).strip()
    extracted_text = cleaned_extracted_text  # Use the cleaned text from now on
    text_lower = extracted_text.lower()  # Update text_lower based on cleaned text

    patterns = {
        # EGAT Information
        "egatAddressTH": r"(?:การไฟฟ้าฝ่ายผลิตแห่งประเทศไทย|กฟผ|กฟผ\.)[:\s]*([\s\S]*?)(?=\d{5}\s*(?:|โทร|tel|fax|โทรสาร|เว็บไซต์|web|email|เลขประจำตัวผู้เสียภาษี|tax id|$)|(?:phone|fax|เลขประจำตัวผู้เสียภาษี|tax id|$))",
        "egatAddressENG": r"(?:electricity generating authority of thailand|egat)[:\s]*([\s\S]*?)(?=\d{5}\s*(?:|phone|fax|web|email|tax id|$)|(?:โทร|โทรสาร|เลขประจำตัวผู้เสียภาษี|tax id|$))",
        "egatTaxId": r"(?:egat tax id|เลขประจำตัวผู้เสียภาษีกฟผ|เลขประจำตัวผู้เสียภาษี)\s*[:]?\s*(\d{10,15})",

        # Gas Station Information
        "gasName": r"(?:station|สาขา|ปั๊ม)[:\s]*([a-zA-Z0-9\s,.-]+)",
        # Note: gasProvider is typically derived from merchantName or a broader check, not a direct regex.
        "gasAddress": r"(ที่อยู่|address)[:\s]*(.*?)(?=\d{5}|\n|$)",
        "gasTaxId": r"(?:tax id|เลขประจำตัวผู้เสียภาษี)[:\s]*(\d{10,15})",
        # Added more gas types
        "gasType": r"(e20|e85|gasohol\s*95|ดีเซล|เบนซิน|แก๊สโซฮอล์|hi\s*diesel\s*s|แก๊สโซฮอล์91|แก๊สโซฮอล์95)",

        # Receipt Details
        # Added TID as a keyword
        "receiptNo": r"(?:receipt no\.?|เลขที่|TID)[:\s]*([a-zA-Z0-9\-/]{5,})",
        "transactionDate": r"(?:date|วันที่|issued|time|วัน)[\s:]*(\d{1,2}[/\-.]\d{1,2}[/\-.]\d{2,4}|\d{4}[/\-.]\d{1,2}[/\-.]\d{1,2})",
        "totalAmount": r"(?:total|amount|ยอดรวม|รวม|ยอดชำระ|รวมเงิน)[:\s]*(\d{1,3}(?:[.,]\d{3})*(?:[.,]\d{1,2})?)",
        "liters": r"(\d+(?:\.\d+)?)\s*(?:ลิตร|litres|liters|l\.)",
        "VAT": r"(?:vat|ภาษีมูลค่าเพิ่ม)[:\s]*(\d+(?:[.,]\d{3})*(?:[.,]\d{1,2})?)",

        # Vehicle Information
        "plateNo": r"([0-9]{1,2}\s*[ก-ฮa-zA-Z]{1,2}\s*[0-9]{3,4})",
...
if __name__ == '__main__':
    ...
    # Get receipt type from command-line arguments
    receipt_type = sys.argv[1]
    # Get original filename from command-line arguments
    ...
    try:
        ...
        # Full OCR text extraction (already done internally for regex pass, but can re-extract for full output)
        # Note: This uses the processed image, not the raw one, for better OCR quality.
        extracted_text = pytesseract.image_to_string(
            Image.fromarray(cv2.cvtColor(np.array(original_image_pil.convert(
                'L')), cv2.COLOR_GRAY2BGR)),  # Ensure it's grayscale for full text
            lang=OCR_LANGUAGES
        )

        # --- Clean raw extracted text here before returning ---
        # Replace multiple whitespaces (spaces, tabs, newlines) with a single space
        ...
        result = {
            ...
            'extracted_text': extracted_text,    # Full raw OCR text (cleaned)
        ...
        print(json.dumps(
            {"error": "Tesseract OCR engine not found. Please install it and ensure its path is correct."}), file=sys.stderr)
        sys.exit(1)
