...
def extract_with_keywords(data, image_cv, result):
    ...
    specifically for KTBCP (Krungthai - Bangchak) receipts.
    """
    keywords = {
        'merchant_name': ['Krungthai', 'bangchak', 'BGN-CHARANSANITWONG'],
        'date': ['DATE', 'วันที่', 'Date'],
        'total_amount': ['AMOUNT THB', 'TOTAL THB', 'รวมเงิน'],
        'gas_provider': ['bangchak'],
        # Parts of the address
        'gas_address': ['CHARANSANITWONG', 'BANGKOK', 'PHRAKHANONG'],
        'gas_tax_id': ['TAX ID', 'เลขประจำตัวผู้เสียภาษี'],
        # TID and TRACE are prominent
        'receipt_no': ['TID', 'TRACE', 'APP.R', 'SALE'],
        'liters': ['LITER', 'L.', 'LTR'],
        'plate_no': ['ทะเบียนรถ'],
        ...
        for field, field_keywords in keywords.items():
            ...
            if any(kw.lower() in word.lower() for kw in field_keywords):
                ...
                if field in ["total_amount", "VAT", "liters", "milestone"]:
                    ...
                    # APP.R is also a number
                    elif 'APP.R' in word and next_word_idx < len(data['text']):
                        appr_match = re.search(
                            r'(\d{6,})', data['text'][next_word_idx])
                        if appr_match:
                            value = appr_match.group(1)
                    else:
                    ...
                    if 'bangchak' in value.lower() or 'บางจาก' in value.lower():
                        value = 'Bangchak'
                    elif 'krungthai' in value.lower() or 'กรุงไทย' in value.lower():
                        if collected['gas_provider'] == "N/A":
                            # Krungthai is bank, not gas provider
                            collected['gas_provider'] = 'Krungthai'
                        if collected['merchant_name'] == "N/A":
                            # Combine for clarity
                            collected['merchant_name'] = 'Bangchak - Krungthai'
                        value = None
                    elif 'charansanitwong' in value.lower() or 'bangkok' in value.lower():  # Part of address
...
def extract_with_regex_patterns(extracted_text, result):
    ...
    patterns = {
        "merchant_name": r"(krungthai\s+bangchak\s+bgn-charansanitwong\s+bangkok|bangchak\s+charansanitwong)",
        # DD/MM/YYYY is common
        "date": r"(?:date|วันที่ขาย|issue date)[:\s]*(\d{1,2}[/\-.]\d{1,2}[/\-.]\d{2,4})",
        "total_amount": r"(?:amount thb|total thb|รวมเป็นเงิน|รวมเงิน)[:\s]*(\d{1,3}(?:[.,]\d{3})*(?:[.,]\d{1,2})?)",
        "gas_provider": r"(bangchak)",
        "gas_address": r"(?:bangchak green net co.,ltd|charansanitwong|phrakhanong|bangkok\s+10260)(?:[\s\S]*?)(?:head office|branch number\s+\d{3})",
        "gas_tax_id": r"(?:tax id|เลขประจำตัวผู้เสียภาษี)[:\s]*(\d{10,15})",
        # TID, TRACE, APPR appear as receipt numbers
        "receipt_no": r"(?:tid|trace|appr)[:\s]*([A-Z0-9\-/]{6,})",
        "liters": r"(\d+(?:\.\d+)?)\s*(?:liter|l\.|ltr)",
        ...
        # EGAT Info (if present, as it appears on the bottom)
        ...
        if is_placeholder_or_na:
            ...
            if match:
                ...
                if field in ["total_amount", "VAT", "liters", "milestone", "egat_tax_id", "gas_tax_id"]:
                    ...
                    if 'krungthai bangchak' in value.lower():
                        value = 'Krungthai - Bangchak'
